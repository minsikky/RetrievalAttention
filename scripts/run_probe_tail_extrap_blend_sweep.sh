...
run_one() {
  ...
  if [[ -f "${out_dir}/summary.json" ]]; then
    echo "[extrap_blend_sweep] skip existing ${name}"
    return
  fi
  echo "[extrap_blend_sweep] run ${name}"
  .venv/bin/python benchmark/selector_eval/runners/run_layer_quality_eval.py \
    ...
    --tail_mode pq_value \
    --tail_blend 1.0 \
    --tail_blend_rule probe_extrapolated \
    --tail_blend_extrap_max "${cap}" \
    --tail_samples 0 \
    --page_size 5632 \
    --subvecs 4 \
    --subbits 6 \
...
run_one probe_tail_p18432_l050_probeex_cap2_128k_v1 18432 2.0
run_one probe_tail_p18432_l050_probeex_cap4_128k_v1 18432 4.0
run_one probe_tail_p18432_l050_probeex_cap8_128k_v1 18432 8.0
run_one probe_tail_p17408_l050_probeex_cap4_128k_v1 17408 4.0
run_one probe_tail_p19456_l050_probeex_cap4_128k_v1 19456 4.0
