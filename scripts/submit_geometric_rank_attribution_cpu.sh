...
tag="${TAG:-20260519}"
output_root="${OUTPUT_ROOT:-attention_efficiency_result/geometric_rank_attribution_cpu_${tag}}"
slurm_root="${SLURM_ROOT:-slurm_out/geometric_rank_attribution_cpu_${tag}}"
manifest="${MANIFEST:-notes/slurm_manifests/geometric_rank_attribution_cpu_${tag}.tsv}"
...
submit_case() {
  ...
  local jobid
  jobid="$(sbatch --parsable --job-name="${label}" --output="${log_path}" --export="${export_args}" scripts/run_geometric_rank_attribution_cpu_one.sh)"
