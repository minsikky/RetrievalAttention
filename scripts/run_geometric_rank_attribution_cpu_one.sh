...
out_dir="${OUTPUT_ROOT:-attention_efficiency_result/geometric_rank_attribution_cpu_20260519}/${name}"
mkdir -p "${out_dir}"

echo "[geom_rank_diag_cpu] host=$(hostname)"
echo "[geom_rank_diag_cpu] started=$(date --iso-8601=seconds)"
echo "[geom_rank_diag_cpu] out=${out_dir}"
echo "[geom_rank_diag_cpu] selector=${SELECTOR_MODE:-fullscan} rule=${ONLINE_CONFIDENCE_RULE:-geometric_probe_tail_switch}"
...
echo "[geom_rank_diag_cpu] finished=$(date --iso-8601=seconds)"
