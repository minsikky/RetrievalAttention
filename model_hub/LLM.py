...
class LLM:
    ...
    def inference(self, inputs_ids, do_sample=False, temperature=0.6, top_p=0.95, top_k=20, ignore_eos=True):
        ...
        decode_gen = max(0, len(outputs_ids) - 1)
        decode_total = max(1e-9, (decode_end - decode_start))
        if decode_gen > 0:
            print(colored(
                f"Decoding latency: {round((decode_end - decode_start), 4)} s ({round((decode_end - decode_start) * 1000 / decode_gen, 2)} ms/step), "
                f"Throughput: {round(self.batch_size * decode_gen / decode_total, 2)} tokens/s",
                'green'
            ))
        else:
            print(colored("Decoding skipped (max_new_length <= 1)", 'green'))
