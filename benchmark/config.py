...
def parse_attn_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Fixed RetrievalAttention token budget. If set (>0), takes priority over ratio.",
    )
...
def generate_config(
    ...
    if attn_type == 'RetrievalAttention':
        static_total = original_config[attn_type]["static_pattern_start"] + original_config[attn_type]["static_pattern_end"]
        dynamic_span = max(1, context_len - static_total)
        if token_budget_override is not None and token_budget_override > 0:
            token_budget = int(token_budget_override)
        elif token_budget_ratio is not None:
            token_budget = max(1, int(dynamic_span * token_budget_ratio))
        else:
            # Legacy default behavior: derive from nprobe and average cluster size.
            avg_cluster_size = max(1, int(dynamic_span / n_clusters))
